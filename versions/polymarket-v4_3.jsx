...
// ═══════════════════════════════════════════════════════════════════════
//  POLYMARKET V4.3 — CORRECTNESS + DETERMINISM + LIFECYCLE HARDENING
//
//  Patch from V4.2:
//   P1. ORDER FSM COMPLETENESS
//       - Terminal states (FILLED/CANCELLED/REJECTED/REPLACED) are immutable
//       - canTransition guards all paths; terminal→any always returns false
//       - Order IDs are deterministic with tick-local sequence counter
//       - Duplicate lifecycle application is idempotent
//       - Order status always matches cumulative fill quantity
//       - REPLACED creates real new order with deterministic ID + lineage
//
//   P2. PARTIAL FILL CORRECTNESS
//       - RETRY: deterministic follow-up children, budget-tracked
//       - REPLACE: new order with deterministic ID, old order terminal
//       - UNWIND: real unwind order when drift exceeds threshold
//       - CANCEL: proper terminal state, all pending children cancelled
//       - All paths maintain fill/position/history consistency
//
//   P3. DUPLICATE FILL PROTECTION
//       - Child-level fill key dedup in advanceOrderFills
//       - Fill ledger append-only, never mutated
//       - Cumulative fills deterministic under replay
//       - fillKeys rebuilt from fills during reconciliation
//
//   P4. RECONCILIATION + LEDGER
//       - Fills are source of truth for positions and realized PnL
//       - Positions rebuilt and verified every tick
//       - Detects: qty mismatches, stale active, orphan fills,
//         replacement lineage breaks, unwind lineage breaks,
//         stale fill keys, duplicate fills in ledger
//       - Corrects position drift automatically
//
//   P5. CIRCUIT BREAKER HARDENING
//       - recentRejects converted to windowed array (was counter)
//       - Poor fill detection only fires for fully-evaluated orders
//       - All 6 triggers config-driven with windowed inputs
//       - half_open probe: deterministic recovery criteria
//       - All transitions auditable via cb.triggers[]
//       - tripCB is pure (returns new CB, no mutation)
//
//   P6. DETERMINISM + REPLAY SAFETY
//       - Order IDs: tick-local sequence counter eliminates collisions
//       - Fill timestamps use injected tickTime only
//       - No hidden mutation leaks in engine functions
//       - Same input state + same time + same events = same output
//       - All windowed arrays bounded
//
//   P7. TARGETED BUG FIXES
//       - Spawn queue: maxSpawnDepth + maxSpawnsPerTick enforced
//       - History pruning: protected lineage retention
//       - CB reject tracking: windowed (was unbounded counter)
//       - Fill timestamp consistency: all fills use tickTime
//       - Deferred spawn execution: deterministic resume
//       - processOrder: poor fill only for completed evaluation
//
//  Preserved from V4.2:
//   [1] Realized PnL: weighted avg cost
//   [2] YES/NO complementary: net/gross exposure
//   [3] Order FSM: 7 states (hardened)
//   [4] Partial fills: RETRY/REPLACE/UNWIND/CANCEL (hardened)
//   [5] MetaAlpha: realized PnL only
//   [7] Slippage: maxSlipBps enforcement
//   [8] Clock: injected time
//   [9] Reconciliation: idempotent fill dedup (hardened)
...
function advanceOrderFills(order, rng, mkts, tickTime, existingFillKeys) {
  ...
  for (const ch of o.children) {
    ...
    if (rng() < fr) {
      ...
      newFills.push({ key: fillKey, orderId: o.id, cid: o.cid, side: o.side, qty: ch.sz, px: rawFP, time: tickTime, slipBps: slip.slipBps });
    }
  }
    ...
    <div style={{ background: K.bg, color: K.tx, minHeight: "100vh", fontFamily: SS, padding: 14 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: "linear-gradient(135deg," + K.g + "," + K.c + ")", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 900, color: K.bg, fontFamily: FF }}>4.3</div>
          <div><div style={{ fontSize: 14, fontWeight: 700 }}>Polymarket V4.3</div>
            <div style={{ fontSize: 8, color: K.dm, fontFamily: FF }}>FSM HARDENED · PURE CB · WINDOWED REJECTS · CHILD FILL DEDUP · DETERMINISTIC IDS</div></div>
        </div>
      ...
      {tab === "System" && <div>
        ...
        <div style={{ ...cd2, fontSize: 8, fontFamily: FF, color: K.dm }}>
          <b style={{ color: K.tx }}>V4.3 correctness guarantees:</b><br />
          [P1] FSM: terminal states immutable. FILLED/CANCELLED/REJECTED/REPLACED cannot transition. Deterministic IDs (seq counter).<br />
          [P2] Partial fills: RETRY (budget-tracked) · REPLACE (new order + lineage) · UNWIND (real order) · CANCEL (terminal)<br />
          [P3] Fill dedup: child-level key dedup in advanceOrderFills. Append-only ledger. Replay-safe.<br />
          [P4] Reconciliation: positions rebuilt from fills every tick. Lineage checks. Fill key consistency.<br />
          [P5] CB: pure functions. recentRejects windowed (was counter). Poor fills evaluated only when all children terminal.<br />
          [P6] Determinism: same input + same time = same output. Order IDs use tick-local sequence counter.<br />
          [P7] Hardening: spawn depth/count limits. History pruning protects lineage chains. Deferred spawn dedup.
        </div>
      </div>}
      ...
      <div style={{ textAlign: "center", padding: "10px 0 4px", fontSize: 7, color: K.dm, fontFamily: FF }}>V4.3 · SEED:{st.seed} · TICK:{st.tickCount} · SEQ:{st.orderSeq} · REALIZED:{f$(st.realizedPnl)} · UNREALIZED:{f$(st.unrealizedPnl)} · NOT FINANCIAL ADVICE</div>
    </div>
  );
}
